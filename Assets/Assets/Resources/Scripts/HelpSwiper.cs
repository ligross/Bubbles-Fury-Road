...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IDragHandler {
    ...
    public void OnDrag(PointerEventData aEventData) {
        ...
        if (Mathf.Abs(aEventData.delta.x) > deltaSwap && !_dragging)
        {
            _dragging = true;
            Debug.Log(aEventData);
            float difference = aEventData.delta.x;
            //if (_horizontal) {
            //    difference = _startPosition.x - _container.anchoredPosition.x;
            //} else {
            //    difference = - (_startPosition.y - _container.anchoredPosition.y);
